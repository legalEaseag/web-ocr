...
def process(init_filename):
    load_path = os.path.join(
            app.config['UPLOAD_FOLDER'],
            init_filename
        )
    
    output = reader.readtext(str(load_path))
    output = ' '.join([tup[1] for tup in  output])
    
    # Update file

    out_filename = init_filename.split('.')[0] + '.txt'
...
def upload():
    ...
    p = Process(target=process, args=[ss_filename])
    p.start()
...
def download_file():
    ss_filename = request.cookies.get('token') + '.txt'
    ...
    filename = request.cookies.get('token') + '.txt'
    response = make_response(send_file(path, as_attachment=True))
    response.set_cookie('filename', 'ocr-output.txt', path='/')
    return response