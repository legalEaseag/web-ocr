...
class Service:
    def __init__(self, token, input_extension=None):
        ...
        self.allowed_mime_types = ["image/png", "image/jpeg", "text/plain"]

        # TODO: use os.path.join() also to add the extension
    ...
    def process(self, **kwargs):
        ...
        output = reader.readtext(self.path_input_file)
        output = " ".join([tup[1] for tup in output])

        # Update file
        with open(self.path_processed_file, "w") as f:
            f.write(output)
